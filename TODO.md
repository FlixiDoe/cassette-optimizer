...
- **Implementation:** Add a numeric setting input: `Leader Tape Delay (seconds)`. When clicking `Start Side`, the UI displays a dedicated countdown, for example `Advancing past leader tape...`, while the user starts the deck. The actual Spotify playback command is delayed until the countdown reaches 0.
...
- **Implementation:** Add a `Level Check` button in the setup panel. Use the native browser **Web Audio API** to synthesize a continuous sine wave tone (400 Hz or 1 kHz at 0 dBFS) or pink noise directly through the active audio interface output. This allows the user to set their deck's physical record input levels accurately before rolling tape.
- **Suggested Commit:** `feat: add web audio test tone generator for level checks`

### 8. Add tape slack margin tolerance to cassette logic

Physical cassettes often contain 1–2 minutes of extra tape length beyond the official specification, for example a C60 side is often about 31 minutes. The strict optimizer logic should account for this variance.

- **Implementation:** Add a configuration field `Tape Slack Margin (seconds)` with a default of 60 seconds. The split algorithm uses this buffer as an artificial headroom extension for Side A/B limits, preventing tight tracks from being unnecessarily pushed to the next tape or side.
...
### 9. Add smart track title cleanup and overrides for J-Cards
...
### 10. Add dynamic color extraction for J-Card themes
...
### 11. Add remaining blank tape duration display
...
### 12. Improve mobile LAN monitor layout
