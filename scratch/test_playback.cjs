...
contains("Dry Run skips Spotify polling", "if (!state.dryRun) startPollingPlayback();");
contains("Dry Run allows recording controls without token", "const needsToken = !state.dryRun");
containsHtml("Lead-in calibration input", 'id="leadInDelay"');
containsHtml("Motor latency calibration input", 'id="motorLatency"');
...
containsHtml("Tape plan selector", 'id="tapePlanSelect"');
containsHtml("Print all J-Cards button", 'id="printAllJCardsBtn"');
...
contains("Multi-tape splitter", "function splitTracksIntoTapes");
contains("Tape plan selector renderer", "function renderTapePlanSelector");
contains("J-Card print stack renderer", "function renderJCardPrint");
contains("J-Card volume titles", "Vol. ${layout.number}");
containsStyles("J-Card print stack CSS", ".jcard-print-stack");
